...
public class TrackingSpaceGenerator
{
    ...
    public static void GenerateRectangleTrackingSpace(int obstacleType, out List<Vector2> trackingSpacePoints, out List<List<Vector2>> obstaclePolygons, out List<InitialConfiguration> initialConfigurations, float width,float height)
    {
        ...
        switch (obstacleType)
        {
            ...
            case 1:
                // 创建一个边长为1m的正方体障碍物
                float halfSide = 0.5f; // 半边长为0.5m，总边长为1m
                obstaclePolygons.Add(new List<Vector2> {
                    new Vector2(-halfSide, halfSide),  // 左上
                    new Vector2(-halfSide, -halfSide), // 左下
                    new Vector2(halfSide, -halfSide),  // 右下
                    new Vector2(halfSide, halfSide)    // 右上
                });
                break;
            case 2:
                float halfSide2 = 0.25f; // 半边长为0.25m，总边长为0.5m
                float offset = 1.5f;
                
                for(int i = -1; i <= 1; i += 2)
                    for(int j = -1; j <= 1; j += 2)
                    {
                        var centerX = i * offset;
                        var centerY = j * offset;
                        obstaclePolygons.Add(new List<Vector2> {
                            new Vector2(centerX - halfSide2, centerY + halfSide2),  // 左上
                            new Vector2(centerX - halfSide2, centerY - halfSide2),  // 左下
                            new Vector2(centerX + halfSide2, centerY - halfSide2),  // 右下
                            new Vector2(centerX + halfSide2, centerY + halfSide2)   // 右上
                        });
                    }
                break;
            default:
