...
public class GlobalConfiguration : MonoBehaviour
{
    ...
    // 使用可配置的generatedPathLength动态计算
    private float pathCircleRadius { get { return generatedPathLength / 2 / Mathf.PI; } }
    ...
    // 添加当前随机种子变量
    private int currentRandomSeed = VirtualPathGenerator.RANDOM_SEED;
    ...
    [Header("Path")]
    [Tooltip("虚拟路径生成的总长度（米）")]
    [Range(50, 1000)]
    public float generatedPathLength = 400;//procedurally generated path length
    ...
    public float distanceToWaypointThreshold = 0.3f;

    [Tooltip("是否限制虚拟路径点在矩形区域内")]
    public bool limitVirtualWaypointsInRectangle = false;
    
    [Tooltip("虚拟路径点矩形区域的X最小值")]
    public float virtualWaypointMinX = -10f;
    
    [Tooltip("虚拟路径点矩形区域的X最大值")]
    public float virtualWaypointMaxX = 10f;
    
    [Tooltip("虚拟路径点矩形区域的Z最小值")]
    public float virtualWaypointMinZ = -10f;
    
    [Tooltip("虚拟路径点矩形区域的Z最大值")]
    public float virtualWaypointMaxZ = 10f;
    ...
    {        
        ...
        // 添加AvatarLayerManager组件，确保Avatar层存在
        if (GetComponent<AvatarLayerManager>() == null) {
            gameObject.AddComponent<AvatarLayerManager>();
        }
    ...
    public void SwitchPersonView(bool ifFirstPersonView) {
        foreach (var avatar in redirectedAvatars)
        {
            ...
            // 在HMD模式下，确保avatar身体的Layer设置正确
            if (movementController == MovementController.HMD) {
                var headFollower = avatar.transform.Find("Body").GetComponent<HeadFollower>();
                if (headFollower != null) {
                    // 确保创建了avatar可视化
                    headFollower.CreateAvatarViualization();
                    // 设置avatar身体的层级
                    headFollower.SetAvatarBodyLayer();
                    
                    // 查找所有HMD相机并设置剔除遮罩
                    var cameraRig = avatar.transform.Find("[CameraRig]");
                    if (cameraRig != null) {
                        ...
                        foreach (var cam in cameras) {
                            ...
                            Debug.Log($"已设置HMD相机 {cam.name} 剔除Avatar层");
                        }
                    }
                }
            }
    ...
    // 修改为接受可选种子参数
    public void InitRandomState(int seed = -1) {
        if (seed < 0)
            seed = VirtualPathGenerator.RANDOM_SEED;
        Random.InitState(seed);
        Debug.Log("随机状态初始化，种子: " + seed);
    }
    ...
    /*  添加一个新方法RegeneratePathsWithNewSeed，用于重新生成路径
        这个修改将实现每次实验生成不同的路径，它会：
        1. 使用系统当前时间的计时器值作为随机种子，确保每次都不同
        2. 用这个新的随机种子初始化随机数生成器
        3. 重新生成所有类型的路径并存储在字典中
        4. 对生成的路径进行随机旋转
        在 StartNextExperiment() 方法中调用这个新方法，确保每次开始新实验时都会重新生成路径
        还添加了一个日志输出，显示使用的新随机种子值，方便调试
    */
    private void RegeneratePathsWithNewSeed()
    {
        // 使用系统时间作为随机种子，确保每次都不同
        ...
        // 保存当前随机种子
        currentRandomSeed = newSeed;
        Debug.Log("路径已使用新的随机种子重新生成: " + newSeed);
    }
    ...
    void EndExperiment(int endState)
    {
        ...
        string tmpFileName = experimentSetupsListIterator + "-" + experimentSetupsList.Count + " " + experimentIterator + "-" + experimentSetups.Count + ".txt";
        string tmpFilePath = Path.Combine(statisticsLogger.Get_TMP_DERECTORY(), tmpFileName);
    ...
    public void GenerateTrackingSpace(int avatarNum, out List<Vector2> trackingSpacePoints, out List<List<Vector2>> obstaclePolygons, out List<InitialConfiguration> initialConfigurations)
    {
        ...
        switch (trackingSpaceChoice)
        {
            ...
            case TrackingSpaceChoice.FilePath:
                ...
                break;
            case TrackingSpaceChoice.Square://允许在Square中使用障碍物
                TrackingSpaceGenerator.GenerateRectangleTrackingSpace(obstacleType, out trackingSpacePoints, out obstaclePolygons, out defaultInitialConfiguration, squareWidth, squareWidth);
                break;
            default:
    ...
    void OnGUI()
    {
        if (showFPS && movementController == MovementController.HMD)
        {
