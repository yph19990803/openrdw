...
public class MovementManager : MonoBehaviour {
    ...
    public void SwitchPersonView(bool ifFirstPersonView) {
        // 获取第一人称和第三人称视图的游戏对象
        GameObject firstPersonViewObj = redirectionManager.simulatedHead.Find("1st Person View").gameObject;
        GameObject thirdPersonViewObj = redirectionManager.simulatedHead.Find("3rd Person View").gameObject;
        
        // 激活相应的视图
        firstPersonViewObj.SetActive(ifFirstPersonView);
        thirdPersonViewObj.SetActive(!ifFirstPersonView);
        
        // 如果是第一人称视图，设置第一人称摄像机的Culling Mask剔除Avatar层
        if (ifFirstPersonView) {
            Camera firstPersonCam = firstPersonViewObj.GetComponent<Camera>();
            if (firstPersonCam != null) {
                // 剔除Avatar层
                firstPersonCam.cullingMask &= ~(1 << LayerMask.NameToLayer("Avatar"));
            }
            
            // 在HMD模式下，也需要设置HMD相机的剔除遮罩
    ...
    public void UpdateWaypoint()
    {
        ...
        {
            waypointIterator++;
            Vector2 waypointPos = waypoints[waypointIterator];
            
            // 限制waypoint位置在矩形区域内，采用反射方式
            waypointPos = ClampPositionToRectangle(waypointPos);
            
            redirectionManager.targetWaypoint.position = new Vector3(waypointPos.x, redirectionManager.targetWaypoint.position.y, waypointPos.y);
        }
    }

    // 将位置限制在虚拟空间的矩形区域内，采用反射方式
    private Vector2 ClampPositionToRectangle(Vector2 position)
    {
        // 如果不需要限制位置，直接返回原始位置
        if (!generalManager.limitVirtualWaypointsInRectangle)
            return position;
            
        // 使用GlobalConfiguration中的虚拟空间矩形区域边界
        float minX = generalManager.virtualWaypointMinX;
        float maxX = generalManager.virtualWaypointMaxX;
        float minZ = generalManager.virtualWaypointMinZ;
        float maxZ = generalManager.virtualWaypointMaxZ;
        
        // 获取上一个路径点的位置
        Vector2 previousPosition = waypointIterator > 0 ? waypoints[waypointIterator - 1] : position;
        ...
        if (position.x < minX) {
            // 计算与左边界的交点
            float t = (minX - previousPosition.x) / direction.x;
            Vector2 intersection = previousPosition + t * direction;
            
            // 计算反射后的位置
            float remainingDistance = direction.magnitude * (1 - t);
            Vector2 reflectedDirection = new Vector2(-direction.x, direction.y).normalized;
            
            // 返回反射后的新位置
            return intersection + reflectedDirection * remainingDistance;
        }
        else if (position.x > maxX) {
            // 计算与右边界的交点
            float t = (maxX - previousPosition.x) / direction.x;
            Vector2 intersection = previousPosition + t * direction;
        ...
        if (position.y < minZ) {
            // 计算与下边界的交点
            float t = (minZ - previousPosition.y) / direction.y;
            Vector2 intersection = previousPosition + t * direction;
            ...
            // 计算与上边界的交点
            float t = (maxZ - previousPosition.y) / direction.y;
            Vector2 intersection = previousPosition + t * direction;
