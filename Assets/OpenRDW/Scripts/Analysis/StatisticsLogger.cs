...
public class StatisticsLogger : MonoBehaviour {
    ...
    public bool logSampleVariables = true;

    // 添加记录每个avatar是否处于重置状态的列表
    private List<bool> avatarIsResetting;
    ...
    [Tooltip("是否使用自定义采样频率，而不是每帧记录")]
    public bool useCustomSamplingFrequency = false;
    
    [Tooltip("上次采样时间")]
    private float lastPositionSamplingTime = 0;
    ...
    {        
        ...
        // 初始化avatarIsResetting列表
        avatarIsResetting = new List<bool>();
        for (int i = 0; i < globalConfiguration.redirectedAvatars.Count; i++) {
            avatarIsResetting.Add(false);
        }
        
        samplingIntervals = new List<float>();
        lastSamplingTime = globalConfiguration.GetTime();
        //自定义采样频率
        lastPositionSamplingTime = globalConfiguration.GetTime();
    }
    ...
    {
        // 获取当前时间
        float currentTime = globalConfiguration.GetTime();
        
        // 检查是否使用自定义采样频率
        bool shouldSample = !useCustomSamplingFrequency || 
                           (useCustomSamplingFrequency && (currentTime - lastPositionSamplingTime) > (1 / samplingFrequency));
        ...
        for (int i = 0; i < avatarStatistics.Count; i++) {
            ...
            // 只在应该采样时记录位置数据
            if (shouldSample) {
                Vector2 realPos = Utilities.FlattenedPos2D(rm.currPosReal);
                Vector2 virtualPos = Utilities.FlattenedPos2D(rm.currPos);
                
                // 如果不在重置状态，添加到采样缓冲区
                if (!avatarIsResetting[i]) {
                    us.userRealPositionSamplesBuffer.Add(realPos);
                    us.userVirtualPositionSamplesBuffer.Add(virtualPos);
        ...
        // 如果使用自定义采样频率并且应该采样，更新上次采样时间
        // 即使在重置状态下也要更新时间，确保采样频率的一致性
        if (useCustomSamplingFrequency && shouldSample) {
            lastPositionSamplingTime = currentTime;
        }
    }
    ...
    public void LogExperimentSummaryStatisticsResults(List<Dictionary<string, string>> experimentResults)
    {
        ...
        // Create XML File
        string xmlFilePath = Path.Combine(SUMMARY_STATISTICS_DIRECTORY, SUMMARY_STATISTICS_XML_FILENAME + ".xml");
        xmlWriter = XmlWriter.Create(xmlFilePath, settings);
    ...
    public void LogExperimentSummaryStatisticsResultsSCSV(List<ResultOfTrial> experimentResults, string resultDir, string resultFileName)
    {
        ...
        string csvFilePath = Path.Combine(resultDir, resultFileName + ".csv");
    ...
    public void LogExperimentPathPictures(int experimentSetupId) {
        ...
        //Export as png file
        string fileName = string.Format("{0}_{1}_realPath.png", experimentSetupId, Utilities.GetTimeStringForFileName());
        string filePath = Path.Combine(GRAPH_DERECTORY, fileName);
        Utilities.ExportTexture2dToPng(filePath, texRealPathGraph);        
    }
    ...
    {        
        ...
        string filePath = Path.Combine(experimentSamplesDirectory, measuredMetric + ".csv");
    ...
    public void LogAllExperimentSamples(string experimentDecriptorString, List<Dictionary<string, List<float>>> oneDimensionalSamplesMaps, List<Dictionary<string, List<Vector2>>> twoDimensionalSamplesMaps)
    {
        ...
        resultDir = Path.Combine(resultDir, fileName + Path.DirectorySeparatorChar);
        Utilities.CreateDirectoryIfNeeded(resultDir);
        string experimentSamplesDirectory = Path.Combine(resultDir, experimentDecriptorString + Path.DirectorySeparatorChar);
        Utilities.CreateDirectoryIfNeeded(experimentSamplesDirectory);
        ...
        for (var i = 0; i < oneDimensionalSamplesMaps.Count; i++)
        {
            ...
            {
                string userDir = Path.Combine(experimentSamplesDirectory, "userId_" + i + Path.DirectorySeparatorChar);
                LogOneDimensionalExperimentSamples(userDir, oneDimensionalSamples.Key, oneDimensionalSamples.Value);
            }
        }
        ...
        for (var i = 0; i < twoDimensionalSamplesMaps.Count; i++)
        {
            ...
            {
                string userDir = Path.Combine(experimentSamplesDirectory, "userId_" + i + Path.DirectorySeparatorChar);
                LogTwoDimensionalExperimentSamples(userDir, twoDimensionalSamples.Key, twoDimensionalSamples.Value);
            }
        }
    }
    ...
    // 重置开始时调用
    public void Event_Reset_Started(int userId)
    {
        if (state == LoggingState.logging && userId < avatarIsResetting.Count)
        {
            avatarIsResetting[userId] = true;
        }
    }

    // 重置结束时调用
    public void Event_Reset_Completed(int userId)
    {
        if (state == LoggingState.logging && userId < avatarIsResetting.Count)
        {
            avatarIsResetting[userId] = false;
        }
    }
}
